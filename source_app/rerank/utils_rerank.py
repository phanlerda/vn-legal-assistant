...
class RerankUtils:
    """
    Cung cấp các hàm RRF, rerank bằng Cohere và rerank bằng model fine-tune.
    """
    ...
    def reciprocal_rank_fusion(self, documents_nested, k: int = 60):
        """
        Áp dụng Reciprocal Rank Fusion để trộn/xếp hạng các tài liệu từ nhiều nguồn.
        """
        document_scores = {}
        try:
            for result_list in documents_nested:
    ...
    def rerank_documents(self, query: str, documents) -> List[Tuple[str, float]]:
        """
        Sử dụng Cohere để xếp hạng lại các tài liệu dựa trên truy vấn.
        """
    ...
    def rerank_documents_finetune(self, query: str, documents) -> List[Tuple[str, float]]:
        """
        Sử dụng model fine-tune để xếp hạng lại các tài liệu dựa trên truy vấn.
        """
        if len(documents) > 50:
